...
package Ast is
   ...
   type Name is new Ast_Abs with
      record
	 Name_Val : String_Access := null;
      end record;
   function Name_Of_String(S:in String) return Name;
   function String_Of_Name(N:in Name) return String;
   function "="(N1:in Name;N2:in Name) return Bool;
   function "<"(N1:in Name;N2:in Name) return Bool;
   
   -- The type of identifiers
   type Ident is new Ast_Abs with
      record
	 Ident_Val : String_Access  := null;
	 Ident_Idxs : Idxs_Array_Access := null;
      end record;   
   ...
   -- Logical connectives
   type LConn is abstract new Ast_Abs with null record;
   type LConn_Access is access all LConn'Class;
   
   type LNot is new LConn with null record;
   type LImplies is new LConn with null record;
   type LAnd is new LConn with null record;
   type LOr is new LConn with null record;
   type LXor is new LConn with null record;
   type LIff is new LConn with null record;
   type LIfThenElse is new LConn with null record;
